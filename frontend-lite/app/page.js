...
function EmptyOutput() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 rounded-xl border border-dashed border-slate-200">
      ...
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-0.5">Converted Markdown will appear here</p>
      </div>
    </div>
  );
}
  ...
  return (
    <pre className="markdown-pre p-4 pb-8">
      {lines.map((line, i) => {
        if (/^#{1,6}\s/.test(line)) {
          return (
            <span key={i} className="block font-medium" style={{ color: '#B45309' }}>
              {line}{'\n'}
            </span>
          );
        }
        if (/^[-*]{3,}$/.test(line.trim())) {
          return (
            <span key={i} className="block border-b border-slate-200 my-1">
              {'\n'}
            </span>
          );
        }
        if (/^```/.test(line) || /^    /.test(line)) {
          return (
            <span key={i} style={{ display: 'block', color: '#6D28D9' }}>
              {line}{'\n'}
            </span>
          );
        }
        if (/^\s*[-*+]\s/.test(line) || /^\s*\d+\.\s/.test(line)) {
          return (
            <span key={i} style={{ display: 'block', color: '#0369A1' }}>
              {line}{'\n'}
            </span>
          );
        }
        if (/^\|/.test(line)) {
          return (
            <span key={i} style={{ display: 'block', color: '#15803D' }}>
              {line}{'\n'}
            </span>
          );
        }
        return <span key={i} className="block text-zinc-600">{line}{'\n'}</span>;
      })}
    </pre>
  );
}
  ...
  return (
    ...
    >
      ...
      <header className="flex-none flex items-center justify-between px-5 py-3 border-b"
        ...
        <div className="flex items-center gap-3">
            ...
            {logoHover && (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <div>
                    <p className="text-sm font-semibold text-zinc-900 leading-tight">MDify</p>
                ...
                <div className="flex flex-col gap-1.5">
                  <p className="text-[10px] text-zinc-400 uppercase tracking-widest font-semibold">Developer</p>
                  <a
                ...
                >
                  ...
                  <span className="text-[10px] text-zinc-400 group-hover/usecase:text-amber-500 transition-colors">→</span>
                </Link>
              </div>
            )}
          </div>
          ...
          >
            ...
            <p className="text-[10px] text-zinc-400 leading-tight">by DevBehindYou · powered by MarkItDown</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row min-h-0 overflow-hidden">
        ...
        {/* ─────── LEFT: INPUT PANEL ─────── */}
        <div className="md:w-[44%] w-full flex flex-col p-3 md:p-4 gap-3 min-h-0 min-w-0 h-[46vh] md:h-auto border-b md:border-b-0 md:border-r" style={{ borderColor: '#E2E8F0' }}>
          ...
          <div className="flex-1 overflow-y-auto space-y-1.5 min-h-0 pr-0.5">
            {files.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-2 py-8">
                <p className="text-xs text-zinc-500">No files added yet</p>
                <p className="text-[10px] text-zinc-400">Supports up to 10 documents at a time</p>
              </div>
            ) : (
              files.map((fileObj) => (
                <FileRow
